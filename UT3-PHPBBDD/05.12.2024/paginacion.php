...
<?php
	...
	if (mysqli_num_rows($pedidos) > 0) {
		...
		$productos = mysqli_query ($conexion, "SELECT * pro.CodigoProducto, Cantidad, PrecioUnidad, Nombre, Gama FROM DetallePedidos dt INNER JOIN Productos pro ON dt.CodigoProducto = pro.CodigoProducto WHERE CodigoPedido = " . $pedido['CodigoPedido'] . ";");
